...
public class sentenceSimilary {
    public static void main(String[] args) {
        ...
        MarkovSimilary markovSimilary = new MarkovSimilary();
        try {
            List<String> sentenceList1 = HanLP.extractSummary(testContent, 1);
            ...
            for (String line : readLines){
                if (StringUtils.isNotBlank(line)){
                    ...
                    if (StringUtils.isNotBlank(content)){
                        List<String> sentenceList2 = HanLP.extractSummary(content, 1);
                        String str2 = StringUtils.strip(sentenceList2.toString(),"[]");
                        System.out.println("摘要为：" + str2);
                        Double result = markovSimilary.markovSimilary(str1, content);
                        allList.add(result);
                    }
                }
            }
